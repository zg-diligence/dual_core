...
int driver_load(void __iomem *outr, void __iomem *inr)
{
...
#else
	{
		res = request_irq(RT_VBUS_GUEST_VIRQ + _irq_offset,
			 _vbus_isr2, IRQF_ONESHOT, "VMM-BUS", NULL);
		if (res) {
	...
	/* 创建通信队列 */
	_prio_que = rt_prio_queue_create("vbus",
					RT_VMM_RB_BLK_NR, sizeof(struct rt_vbus_pkg));
	if (!_prio_que) {
		res = -ENOMEM;
		goto _free_irq;
	}
	...
	
	{
		...
		for (i = 0; i < ARRAY_SIZE(_chn_wm_que); i++) {
			rt_wm_que_init(&_chn_wm_que[i],
				RT_VMM_RB_BLK_NR / 3, RT_VMM_RB_BLK_NR * 2 / 3);
		}
		...
		for (i = 1; i < ARRAY_SIZE(_chn_recv_wm); i++) {
			rt_vbus_set_recv_wm(i, RT_VMM_RB_BLK_NR / 3, RT_VMM_RB_BLK_NR * 2 / 3);
	...
	OUT_RING = outr;	/* VBUS 读出地址 */
	IN_RING  = inr;		/* VBUS 写入地址 */
